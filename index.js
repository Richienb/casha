...
module.exports = async (amount, from, to, { date = "latest", precision = 2, provider = "exchangeratesapi", apiKey } = {}) => {
	...
	ow(date, ow.any(
		ow.string.is((value) => value === "latest" || day(value).format() !== "Invalid Date")),
	ow.number.is((value) => day(value).format() !== "Invalid Date"),
	ow.date.is((value) => day(value).format() !== "Invalid Date"),
	ow.object.is((value) => day(value).format() !== "Invalid Date"), // Class
	)
	...
	ow(provider, ow.string.is((value) => ["exchangeratesapi", "fixer", "currencylayer", "openexchangerates"].includes(value)))
